...
app.use(
  "/coustom",
  express.static(path.join(__dirname, "../template/coustom"))
);
app.use(
  "/bootstrap",
...
app.post("/weather",async (req, res) => {
  ...
  requests(
    ...
    if(jsonToObj.current == undefined){
      res.render('weather',{err:"Location not found : )"})
...
app.get("/weather", (req, res) => {
  requests(
    `http://api.weatherapi.com/v1/current.json?key=${apiKey}&q=bangladesh&aqi=yes`
  ).on("data", (chunk) => {
    const jsonToObj = JSON.parse(chunk);
    const icon = jsonToObj.current.condition.icon;
    const city = jsonToObj.location.name;
